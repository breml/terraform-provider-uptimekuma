...
- `operation_timeout` (String) Timeout for a single Uptime Kuma operation as a Go duration string (e.g. `30s`, `2m`), applied once the connection is established (default: `60s`). Bounds how long the provider waits for the server to acknowledge and confirm one create, read, update or delete. Unlike `timeout`, which covers connecting, this guards against an Uptime Kuma instance that accepts a command and never answers it, which would otherwise block the apply indefinitely. Can be set via `UPTIMEKUMA_OPERATION_TIMEOUT` environment variable.
