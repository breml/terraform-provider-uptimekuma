...
func (r *NotificationAliyunsmsResource) Read(
	...
	base, found := readNotification(ctx, r.client, id, notification.AliyunSMSDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.AliyunSMSDetails{}.Type()),
			err.Error(),
		)
		return
	}
