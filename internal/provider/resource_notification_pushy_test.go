...
func TestAccNotificationPushyDataSource(t *testing.T) {
	name := acctest.RandomWithPrefix("NotificationPushyDS")
	apiKey := "test-api-key-ds-123456789"
	token := "test-device-token-ds-abc123"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNotificationPushyDataSourceConfig(name, apiKey, token),
				ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue(
...
func testAccNotificationPushyDataSourceConfig(
