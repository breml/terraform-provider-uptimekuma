...
func (r *NotificationGrafanaOncallResource) Read(
	...
	base, found := readNotification(ctx, r.client, id, notification.GrafanaOncallDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.GrafanaOncallDetails{}.Type()),
			err.Error(),
		)
		return
	}
