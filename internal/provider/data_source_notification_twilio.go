...
func (d *NotificationTwilioDataSource) readByID(
	...
	if notification.Type() != "twilio" {
		resp.Diagnostics.AddError("Incorrect notification type", "Notification is not a Twilio notification")
		return
	}
...
func (d *NotificationTwilioDataSource) readByName(
	...
	id, ok := findNotificationByName(ctx, d.client, data.Name.ValueString(), "twilio", &resp.Diagnostics)
	if !ok {
		return
	}
