...
func (r *NotificationWPushResource) Read(
	...
	base, found := readNotification(ctx, r.client, id, notification.WPushDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.WPushDetails{}.Type()),
			err.Error(),
		)
		return
	}
