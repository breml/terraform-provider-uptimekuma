...
func (r *NotificationSquadcastResource) Read(
	...
	base, found := readNotification(ctx, r.client, id, notification.SquadcastDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.SquadcastDetails{}.Type()),
			err.Error(),
		)
		return
	}
