...
func (r *NotificationPumbleResource) Read(
	...
	base, found := readNotification(ctx, r.client, id, notification.PumbleDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.PumbleDetails{}.Type()),
			err.Error(),
		)
		return
	}
