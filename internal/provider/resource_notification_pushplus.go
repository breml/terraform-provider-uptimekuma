...
func (r *NotificationPushPlusResource) Read(
	...
	base, found := readNotification(ctx, r.client, id, notification.PushPlusDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.PushPlusDetails{}.Type()),
			err.Error(),
		)
		return
	}
