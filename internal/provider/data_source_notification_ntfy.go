...
func (d *NotificationNtfyDataSource) readByID(
	ctx context.Context,
	data *NotificationNtfyDataSourceModel,
	resp *datasource.ReadResponse,
) {
...
func (d *NotificationNtfyDataSource) readByName(
	ctx context.Context,
	data *NotificationNtfyDataSourceModel,
	resp *datasource.ReadResponse,
) {
	id, ok := findNotificationByName(ctx, d.client, data.Name.ValueString(), "ntfy", &resp.Diagnostics)
	if !ok {
		return
	}
