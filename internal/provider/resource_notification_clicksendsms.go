...
func (r *NotificationClicksendSmsResource) Read(
	...
	base, found := readNotification(ctx, r.client, id, notification.ClickSendSMSDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.ClickSendSMSDetails{}.Type()),
			err.Error(),
		)
		return
	}
