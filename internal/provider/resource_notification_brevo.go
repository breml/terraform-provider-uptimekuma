...
func (r *NotificationBrevoResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	base, found := readNotification(ctx, r.client, id, notification.BrevoDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.BrevoDetails{}.Type()),
			err.Error(),
		)
		return
	}
