...
func (r *NotificationHomeAssistantResource) Read(
	...
	base, found := readNotification(ctx, r.client, id, notification.HomeAssistantDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.HomeAssistantDetails{}.Type()),
			err.Error(),
		)
		return
	}
