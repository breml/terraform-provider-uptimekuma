...
func (r *NotificationAlertNowResource) Read(
	...
	base, found := readNotification(ctx, r.client, id, notification.AlertNowDetails{}.Type(), &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("failed to convert notification to type %q", notification.AlertNowDetails{}.Type()),
			err.Error(),
		)
		return
	}
